...
long double CheckExcep(double base, double exp) {
  ...
  } else if ((base == -1 && exp == S21_POS_INF) || base == 1) {
    result = 1.0;
  } else if (exp == 0) {
    result = 1;
  } else if (base < 0 && (long)exp != exp && base != S21_NEG_INF &&
             exp != S21_NEG_INF) {
    result = S21_NAN;
...
long double s21_pow(double base, double exp) {
  ...
  int sign = 0;
  if (exp < 0) sign = 1;
  if (CheckExcep(base, exp) != -1) {
    result = CheckExcep(base, exp);
